...
game.ReplicatedStorage.Interaction.ClientIsWhitelisted:InvokeServer(Owner)
game.ReplicatedStorage.Interaction.ClientIsWhitelisted:InvokeServer(SpawnPad.Owner.Value)
...
game.ReplicatedStorage.Interaction.InteractionPermission
...
local ClientPlacedStructure = ReplicatedStorage:WaitForChild("PlaceStructure")
...
local function Notify(msg)
    print("[StructureTest] " .. msg)
    if _G.DynxeNotify then _G.DynxeNotify("StructureTest", msg, 4) end
end

local function GetBlueprintFromTarget(target)
    if not target then return nil end
    local current = target
    while current and current.Parent ~= workspace.PlayerModels do
        current = current.Parent
    end
    if not current then return nil end
    if not current:FindFirstChild("ItemName") then return nil end
    return current
end

local function GetBlueprintCFrame(model)
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.Z then
        local blueprint = GetBlueprintFromTarget(Mouse.Target)

        if not blueprint then
            Notify("No valid blueprint under mouse.")
            selected = { blueprintType=nil, woodClass=nil, placeCFrame=nil, rawModel=nil }
            return
        end

        local cf = GetBlueprintCFrame(blueprint)
        if not cf then
            Notify("Could not read CFrame from blueprint.")
            return
        end

        local woodClass = "nil"
        if blueprint:FindFirstChild("BlueprintWoodClass") then
            woodClass = tostring(blueprint.BlueprintWoodClass.Value)
        end

        selected.blueprintType = tostring(blueprint.ItemName.Value)
        selected.woodClass     = woodClass
        selected.placeCFrame   = cf
        selected.rawModel      = blueprint

        Notify(("Selected: %s | Wood: %s"):format(selected.blueprintType, selected.woodClass))
    end

    if input.KeyCode == Enum.KeyCode.X then
        if not selected.blueprintType then
            Notify("Nothing selected — hover a blueprint and press Z first.")
            return
        end

        local worldCF = selected.placeCFrame + Vector3.new(0, 1, 0)

        Notify(("Firing | %s | Wood: %s"):format(selected.blueprintType, selected.woodClass))

        ClientPlacedStructure:FireServer(
            selected.blueprintType,
            worldCF,
            LP,
            selected.rawModel.BlueprintWoodClass.Value,
            selected.rawModel,
            true
        )
...
Notify("Ready. Z = select blueprint | X = fire remote.")
