...
function getTreePos(tree)
	for _,branch in tree:GetChildren() do
		if branch.Position then
...
-- Find ESP items and add/remove to/from list
local lastScan = 0
task.spawn(function()
    while true do
        local now = os.clock()
...
UI.AddTab("Lumberboog", function(tab)
    ...
    tree_teleport_Sec:Button("Teleport To Tree", function()
        ...
        setPlayerPosition(getTreePos(selected))
    end)

	-- Teleport log to dropoff
