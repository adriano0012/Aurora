...
return function(UI, Config, Utils)
    ...
    local Tab = UI:Tab(Utils._("player_title"), "5012544693")
    
    local Movement = Tab:Section(Utils._("player_movement"), true)
    local CameraSection = Tab:Section(Utils._("player_camera"), false)
    local Utilities = Tab:Section(Utils._("player_utilities"), false)
    ...
    -- Movement
    Movement:Slider(Utils._("player_walk"), "WalkSpeed", Config.WalkSpeed or 16, 16, 1000, false, function(v)
        Config.WalkSpeed = v
        ApplyMovementValues()
    end)
    
    Movement:Slider(Utils._("player_sprint"), "SprintSpeed", Config.SprintSpeed or 65, 16, 1000, false, function(v)
        Config.SprintSpeed = v
        ApplyMovementValues()
    end)
    
    Movement:Toggle(Utils._("player_sprint_inf"), "SprintInf", Config.SprintInf or false, function(v)
        Config.SprintInf = v
    end)
    
    Movement:Slider(Utils._("player_jump"), "JumpPower", Config.JumpPower or 50, 50, 1000, false, function(v)
        Config.JumpPower = v
        ApplyMovementValues()
    end)
    
    Movement:Slider(Utils._("player_fly_speed"), "FlySpeed", Config.FlySpeed or 200, 50, 800, false, function(v)
        Config.FlySpeed = v
    end)
    
    Movement:Keybind(Utils._("player_fly_key"), "FlyKey", Config.FlyKey or "F", function(v)
        Config.FlyKey = v
    end)
    
    Movement:Keybind(Utils._("player_fly_up"), "FlyUp", Config.FlyUp or "E", function(v)
        Config.FlyUp = v
    end)
    
    Movement:Keybind(Utils._("player_fly_down"), "FlyDown", Config.FlyDown or "Q", function(v)
        Config.FlyDown = v
    end)
    
    Movement:Keybind(Utils._("player_noclip_key"), "NoclipKey", Config.NoclipKey or "LeftControl", function(v)
        Config.NoclipKey = v
    end)
    
    NoclipToggleControl = Movement:Toggle(Utils._("player_noclip"), "Noclip", Config.Noclip or false, function(v)
        Config.Noclip = v
        SetNoclip(v)
    end)
    
    Movement:Toggle(Utils._("player_infinite_jump"), "InfiniteJump", Config.InfiniteJump or false, function(v)
        Config.InfiniteJump = v
        SetInfiniteJump(v)
    end)
    
    Movement:Toggle(Utils._("player_flight"), "Flight", Config.Flight or false, function(v)
        FlightArmed = v
        Config.Flight = v
        if not v then
            SetFly(false)
    ...
    Movement:Toggle(Utils._("player_invisible"), "Invisible", Config.Invisible or false, function(v)
        Config.Invisible = v
        SetInvisible(v)
    end)
    
    -- Camera
    local FOVSlider = CameraSection:Slider(Utils._("player_fov"), "FOV", Config.FOV or 70, 1, 120, false, function(v)
        local camera = Workspace.CurrentCamera
    ...
    CameraSection:Slider(Utils._("player_zoom"), "Zoom", Config.Zoom or 100, 100, 10000, false, ApplyZoom)
    ApplyZoom(Config.Zoom or 100)
    
    CameraSection:Button(Utils._("player_reset_fov"), function()
        local camera = Workspace.CurrentCamera
    ...
    -- Utilities
    Utilities:Toggle(Utils._("player_flashlight"), "Flashlight", Config.Flashlight or false, function(v)
        Config.Flashlight = v
        SetFlashlight(v)
    end)
    
    Utilities:Toggle(Utils._("player_antiafk"), "AntiAFK", Config.AntiAFK ~= false, function(v)
        Config.AntiAFK = v
        SetAntiAFK(v)
    end)
    
    Utilities:Toggle(Utils._("player_godmode"), "GodMode", Config.GodMode or false, function(v)
        Config.GodMode = v
        SetGodMode(v)
    end)
    
    Utilities:Toggle(Utils._("player_antikb"), "AntiKB", Config.AntiKB or false, function(v)
        Config.AntiKB = v
        SetAntiKnockback(v)
    end)
    
    Utilities:Toggle(Utils._("player_antivoid"), "AntiVoid", Config.AntiVoid or false, function(v)
        Config.AntiVoid = v
        SetAntiVoid(v)
    end)
    
    Utilities:Button(Utils._("player_safe_death"), SafeDeath)
    
    Utilities:Button(Utils._("player_btools"), CreateBTools)
    
    Utilities:Toggle(Utils._("player_hard_dragger"), "HardDragger", Config.HardDragger or false, function(v)
